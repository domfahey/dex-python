...
def get_engine(db_path: Path | str | None = None) -> Engine:
    ...
    Parameters:
        db_path: Path to the SQLite database file. If omitted or None,
            DEFAULT_DB_PATH is used.

    Returns:
        SQLAlchemy Engine connected to the specified SQLite database file.
    """
...
def get_session(db_path: Path | str | None = None) -> Session:
    """
    Create a new SQLAlchemy Session bound to the SQLite database.
    ...
    Returns:
        A SQLAlchemy Session instance bound to the engine for the database.
    """
